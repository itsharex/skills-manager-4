...
import { searchMarket, installSkill, openURL } from "../bridge";
import type { MarketSearchResult, MarketSearchSkill, InstallResult } from "../types";

const SOURCE_CONFIG: Record<string, { label: string; icon: React.ReactNode }> = {
  pool: { label: "本地技能池", icon: <Database className="h-4 w-4 text-blue-500" /> },
  clawhub: { label: "ClawHub", icon: <Sparkles className="h-4 w-4 text-purple-500" /> },
  skillssh: { label: "skills.sh", icon: <Globe className="h-4 w-4 text-emerald-500" /> },
  github: { label: "", icon: <Globe className="h-4 w-4 text-gray-600" /> },
  registry: { label: "", icon: <Globe className="h-4 w-4 text-orange-500" /> },
};
...
export default function MarketPage({ onRefresh }: Props) {
  ...
  const [installing, setInstalling] = useState<string | null>(null); // skill name being installed
  ...
  const handleInstall = async (skill: MarketSearchSkill) => {
    if (selectedAgents.length === 0) {
      setInstallLog(["请先选择目标智能体"]);
      return;
    }

    setInstalling(skill.name);
    try {
      // Construct source URL from skill info
      const sourceUrl = buildSourceUrl(skill);
      const res: InstallResult[] = await installSkill(sourceUrl, {
        namespace: skill.namespace,
        agents: selectedAgents,
      });
      setInstallLog(prev => [
        ...prev,
        ...res.map(r => `✅ ${r.name}@${r.version} → ${r.synced ? r.storePath : r.error}`),
      ]);
      onRefresh();
    } catch (e: any) {
      setInstallLog(prev => [...prev, `❌ ${skill.name}: ${e.message}`]);
    } finally {
      setInstalling(null);
    }
  };

  const buildSourceUrl = (skill: MarketSearchSkill): string => {
    if (skill.source) {
      // For ClawHub: owner/slug, for GitHub: owner/repo
      return `https://github.com/${skill.source}`;
    }
    return `https://github.com/${skill.namespace}/${skill.name}`;
  };
  ...
  const buildExternalUrl = (skill: MarketSearchSkill, sourceType: string): string | null => {
    ...
    switch (sourceType) {
      ...
      case "github":
        return `https://github.com/${skill.source}`;
      case "registry":
        // Registry might have a web URL in source; fallback to HTTP
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            搜索范围：本地技能池、ClawHub、skills.sh{/* + 配置的 GitHub 仓库 */}
          </p>
        </CardContent>
      </Card>
      ...
      {/* Results */}
      {hasSearched && !searching && totalSkills > 0 && (
        <>
          {/* Agent Selector + Install Summary */}
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-base">
                搜索结果
                <Badge variant="secondary" className="ml-2">{totalSkills} 个技能</Badge>
              </CardTitle>
            </CardHeader>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-muted/50 border-b">
                              <th className="text-left font-medium px-4 py-2 w-[30%]">名称</th>
                              <th className="text-left font-medium px-4 py-2 w-[15%]">版本</th>
                              <th className="text-left font-medium px-4 py-2 w-[25%]">来源</th>
                              <th className="text-left font-medium px-4 py-2 w-[20%]">描述</th>
                              <th className="text-right font-medium px-4 py-2 w-[10%]"></th>
                            </tr>
                          </thead>
                          <tbody>
                            {skills.map((skill, si) => (
                              <tr key={si} className="border-b last:border-0 hover:bg-muted/30">
                                ...
                                <td className="px-4 py-2">
                                  <span className="text-xs text-muted-foreground line-clamp-2">
                                    {skill.description || (skill.installs ? `${formatInstalls(skill.installs)} 安装` : "")}
                                  </span>
                                </td>
